...
export const ImportCategoriesModal: React.FC<ImportCategoriesModalProps> = ({ isOpen, onClose }) => {
  ...
  const [parsedRows, setParsedRows] = useState<ParsedCategoryRow[] | null>(null);
  ...
  const processFile = async (file: File) => {
    ...
    try {
      const rows = await parseCategoriesFromExcel(file, masterCategories);
      setParsedRows(rows);
    } catch (err: any) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      setParseError('Failed to import categories. Please try again.');
    } finally {
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-4xl flex flex-col overflow-hidden max-h-[90vh]">
        ...
        <div className="flex items-center justify-between p-6 border-b border-border bg-surface-muted/30">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-text">Bulk Import Categories</h2>
              <p className="text-sm text-secondary">Upload an Excel file to create or update categories</p>
            </div>
          </div>
          <button 
            onClick={onClose}
          ...
          {importSuccess ? (
            <div className="py-12 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-secondary mb-6 max-w-sm">
                Successfully imported {validRows} categories ({newCount} new, {updateCount} updated). The category master catalog has been updated.
              </p>
            </div>
          ) : (
            <>
                  ...
                  <div className="bg-primary/5 border border-primary/20 rounded-xl p-5 flex gap-4">
                    ...
                    <div>
                      ...
                      <ul className="text-sm text-primary-dark/80 space-y-1.5 list-disc list-inside">
                        ...
                        <li><strong>New Categories:</strong> Leave the "Category ID" column blank.</li>
                        <li><strong>Update Existing:</strong> Provide the exact "Category ID". If the name and type match exactly, it will also update.</li>
                      </ul>
                      <button 
                        onClick={downloadSampleCategoryTemplateExcel}
                                ...
                                <td className="py-2 px-3 text-secondary">
                                  {row.color ? (
                                    <div className="flex items-center gap-1.5">
                                      <div className="w-3 h-3 rounded-full border border-border" style={{ backgroundColor: row.color }} />
            ...
            >
              {isImporting ? (
                ...
                <>
                  ...
                  <span>Import {validRows} Categories</span>
                </>
              )}
            </button>
          </div>
