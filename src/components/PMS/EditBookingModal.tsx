...
export const EditBookingModal: React.FC = () => {
  ...
  useEffect(() => {
    if (bookingToEdit && isEditBookingModalOpen) {
      setRoomNumber(bookingToEdit.roomNumber);
      setGuestName(bookingToEdit.guestName);
      setGuestEmail(bookingToEdit.guestEmail || '');
      setGuestPhone(bookingToEdit.guestPhone || '');
      setGuestCountry(bookingToEdit.guestCountry || '');
      setPassportOrId(bookingToEdit.passportOrId || '');
      setCheckInDate(bookingToEdit.checkInDate);
      setCheckOutDate(bookingToEdit.checkOutDate);
      setChannel(bookingToEdit.channel);
      setAdultsCount(bookingToEdit.adultsCount || 2);
      setKidsCount(bookingToEdit.childrenCount || 0);
      setChildrenAges(bookingToEdit.childrenAges || []);
      setMealPlan(bookingToEdit.mealPlan || 'room_only');
      setSpecialRequests(bookingToEdit.specialRequests || '');
      setCustomRateUSD(bookingToEdit.ratePerNightUSD.toString());
    }
  }, [bookingToEdit, isEditBookingModalOpen]);

  if (!isEditBookingModalOpen) return null;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!guestName.trim() || !bookingToEdit) return;
      ...
      <div className="bg-white rounded-2xl max-w-xl w-full border border-border shadow-2xl p-5 space-y-4 my-8 animate-in fade-in zoom-in duration-150">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          <div className="flex items-center justify-end gap-2 pt-2 border-t border-border">
            ...
            <button
              type="submit"
              className="px-4 py-2 rounded-lg bg-primary text-white font-bold hover:bg-primary-dark shadow-xs cursor-pointer"
            >
              Save Changes
            </button>
          </div>
